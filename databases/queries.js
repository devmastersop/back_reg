...
const qGetByName = 'select c.nom_curso, pro.nom_programa, p.persona_apaterno, p.persona_amaterno, p.persona_nombres, p.persona_dni, d.per_academico, pl.t_horas, pl.t_unidad, pl.importe from persona p INNER JOIN docente d ON d.nro_document=p.persona_dni inner join planilla pl on p.persona_id = pl.persona_id  ' +
...
const qGetById = 'select c.nom_curso, pro.nom_programa, p.persona_apaterno, p.persona_amaterno, p.persona_nombres, p.persona_dni, d.per_academico, pl.t_horas, pl.t_unidad, pl.importe from persona p INNER JOIN docente d ON d.nro_document=p.persona_dni inner join planilla pl on p.persona_id = pl.persona_id  inner join curso c on c.id_curso = pl.id_curso inner join programa pro on c.id_programa = pro.id_programa' + 
